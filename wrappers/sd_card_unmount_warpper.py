...
class SDCardUnmountWarpper(Wrapper):
    ...
    def parse(self) -> None:
        ...
        if tag_name != "SDCardUnmount":
            raise ValueError("Expected !SDCardUnmount command")
        ...
        LOGGER.info(
            "Parsed SDCardUnmount values: name=%s, sd_card_path=%s, wait_after_s=%s",
            self.name,
            self.sd_card_path,
            self.wait_after_s,
        )
    ...
    def execute(self) -> None:
        LOGGER.info("SDCardUnmount wrapper execute called")
