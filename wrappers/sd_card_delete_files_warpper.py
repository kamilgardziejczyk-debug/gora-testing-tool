...
class SdCardDeleteFilesWarpper(Wrapper):
    ...
    def parse(self) -> None:
        ...
        if tag_name != "SdCardDeleteFiles":
            raise ValueError("Expected !SdCardDeleteFiles command")
        ...
        LOGGER.info(
            "Parsed SdCardDeleteFiles values: name=%s, sd_card_path=%s, file_pattern=%s",
            self.name,
            self.sd_card_path,
            self.file_pattern,
        )
    ...
    def execute(self) -> None:
        LOGGER.info("SdCardDeleteFiles wrapper execute called")
