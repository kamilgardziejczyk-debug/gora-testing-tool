...
REQUIRED_FIELDS = ("session", "endpoint", "client_id", "cert", "private_key", "root_ca", "topic")
CERT_FIELDS = ("cert", "private_key", "root_ca")


class MqttSubscribeWrapper(Wrapper):
    """Opens an MQTT session and starts buffering messages on a topic.
    ...
    def parse(self) -> None:
        ...
        LOGGER.info(
            "Parsed MqttSubscribe: name=%s, session=%s, endpoint=%s:%s, client_id=%s, topic=%s, qos=%s",
            self.name,
            self.session,
            self.endpoint,
            self.port,
    ...
    def execute(self) -> None:
        ...
        try:
            ...
            listener.subscribe(self.topic, qos=self.qos)
        ...
        LOGGER.info("MQTT session '%s' is buffering messages on '%s'", self.session, self.topic)
